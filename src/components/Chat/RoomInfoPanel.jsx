...
export default function RoomInfoPanel({ room, onClose }) {
  ...
  const handleBlock = async (uid) => {
    ...
    if (isBlocked) {
      ...
      if (!window.confirm("Block this user? Their messages will be hidden from you.")) return;
      await blockUser(userProfile.uid, uid);
    }
  };
              ...
              {inviteResults.map((u) => (
                <div key={u.uid} className="member-row">
                  <div className="avatar" style={{ width: 32, height: 32, fontSize: 13 }}>
                    {u.photoURL ? <img src={u.photoURL} alt={u.displayName} style={{ width: 32, height: 32, borderRadius: "50%" }} /> : u.displayName?.[0]}
                  </div>
              ...
              {members.map((member) => {
                ...
                return (
                  <div key={member.uid} className="member-row">
                    <div className="avatar" style={{ width: 36, height: 36, fontSize: 14, position: "relative" }}>
                      {member.photoURL
                        ? <img src={member.photoURL} alt={member.displayName} style={{ width: 36, height: 36, borderRadius: "50%" }} />
                    ...
                    {!isYou && (
                      <div className="member-actions">
                        <button
                          className={`btn btn-ghost member-action-btn ${isBlocked ? "unblock" : ""}`}
                          style={{ fontSize: 12, padding: "4px 10px" }}
                          onClick={() => handleBlock(member.uid)}
                        ...
                        {isAdmin && (
                          <button
                            className="btn btn-danger member-action-btn"
                            style={{ fontSize: 12, padding: "4px 10px" }}
