...
import { registerWithEmail, loginWithEmail, loginWithGoogle, checkGoogleRedirectResult } from "../../firebase/auth";
import "./AuthPage.css";
...
export default function AuthPage() {
  ...
  useEffect(() => {
    checkGoogleRedirectResult().catch(console.error);
  }, []);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // On mobile, page redirects — on desktop, result is immediate
    } catch (err) {
      setErrorMessage(convertFirebaseError(err.code));
      setIsSubmitting(false);
    }
    ...
    <div className="auth-page">
      ...
      <div className="auth-card anim-pop-in">
        ...
        <div className="auth-tabs">
          <button className={`auth-tab ${authMode === AUTH_MODE_LOGIN ? "active" : ""}`}
            onClick={() => setAuthMode(AUTH_MODE_LOGIN)}>Sign In</button>
          <button className={`auth-tab ${authMode === AUTH_MODE_SIGNUP ? "active" : ""}`}
            onClick={() => setAuthMode(AUTH_MODE_SIGNUP)}>Create Account</button>
        </div>
        ...
        <form onSubmit={handleEmailSubmit} className="auth-form">
          ...
          <button type="submit" className="btn btn-primary auth-submit" disabled={isSubmitting}>
            {isSubmitting ? <span className="anim-spin">⟳</span>
        ...
        <button className="btn btn-ghost google-btn" onClick={handleGoogleLogin} disabled={isSubmitting}>
          ...
          {isSubmitting ? "Redirecting..." : "Google"}
        </button>
      </div>
    </div>
  );
